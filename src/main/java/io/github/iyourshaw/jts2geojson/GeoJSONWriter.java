...
public class GeoJSONWriter {
  ...
  io.github.iyourshaw.geojson.Point convert(Point point) {
    io.github.iyourshaw.geojson.Point json =
        new io.github.iyourshaw.geojson.Point(convert(point.getCoordinate()));
    return json;
  }

  io.github.iyourshaw.geojson.MultiPoint convert(MultiPoint multiPoint) {
    return new io.github.iyourshaw.geojson.MultiPoint(convert(multiPoint.getCoordinates()));
  }

  io.github.iyourshaw.geojson.LineString convert(LineString lineString) {
    return new io.github.iyourshaw.geojson.LineString(convert(lineString.getCoordinates()));
  }

  io.github.iyourshaw.geojson.LineString convert(LinearRing ringString) {
    return new io.github.iyourshaw.geojson.LineString(convert(ringString.getCoordinates()));
  }
  ...
  io.github.iyourshaw.geojson.GeometryCollection convert(GeometryCollection gc) {
    ...
    io.github.iyourshaw.geojson.Geometry[] geometries =
        new io.github.iyourshaw.geojson.Geometry[size];
    for (int i = 0; i < size; i++) {
